...
import { AgentAuthorizationPanel } from "@/components/sodex/agent-authorization-panel";
import {
  fetchSoDEXReadiness,
  type SoDEXReadinessPayload,
} from "@/lib/sodex-readiness";
import { useVegaAuth } from "@/lib/vega-auth";

export type SoDEXOnboardingStatus = {
  ready: boolean;
  blocker: string | null;
...
export function SoDEXOnboardingChecklist({
  open,
  onClose,
  mode = "builder",
  onStatusChange,
  ...
  onClose: () => void;
  mode?: "builder" | "agent";
  onStatusChange?: (status: SoDEXOnboardingStatus) => void;
  walletAddressOverride?: string | null;
}) {
  const { authenticated, login, walletAddress, getAuthHeaders } = useVegaAuth();
  const resolvedWalletAddress = walletAddressOverride?.trim() || walletAddress || null;
  const [readiness, setReadiness] = useState<SoDEXReadinessPayload | null>(null);
  ...
  const loadReadiness = useCallback(async () => {
    ...
    return fetchSoDEXReadiness(resolvedWalletAddress, getAuthHeaders);
  ...
  useEffect(() => {
    ...
    async function loadReadiness() {
      ...
      setError(null);
      try {
        const payload = await fetchSoDEXReadiness(activeWalletAddress, getAuthHeaders);
        if (cancelled) {
          return;
        }
        ...
        setStatus("error");
        setError(loadError instanceof Error ? loadError.message : "Unable to load SoDEX readiness.");
      }
    }
  ...
  const blocker = useMemo(() => {
    ...
    if (!resolvedWalletAddress) {
      return "Connect the ValueChain wallet you want Vega to trade with.";
    }
    if (resolvedStatus === "loading") {
      return "Checking SoDEX readiness.";
    }
    if (resolvedError) {
          ...
          <div className="grid gap-3 xl:grid-cols-[1.4fr_1fr_1fr_1fr]">
            <article className="grid gap-2 rounded-[1.2rem] border border-[rgba(220,232,93,0.14)] bg-[rgba(220,232,93,0.06)] p-4">
              ...
              <div className="font-mono text-sm font-bold uppercase tracking-tight text-neutral-50">
                {blocker ?? "All SoDEX checks are verified."}
              </div>
            ...
            <article className="grid gap-1 rounded-[1.2rem] border border-[rgba(255,255,255,0.06)] bg-[#121416] p-4">
                <span className="text-[0.56rem] font-semibold uppercase tracking-[0.16em] text-neutral-500">SoDEX equity</span>
                <div className="font-mono text-xl font-bold uppercase tracking-tight text-neutral-50">
                ...
                {step.id === "funding" ? (
                  <div className="grid gap-2 rounded-[1rem] border border-[rgba(255,255,255,0.06)] bg-[#090a0a] p-3 text-xs leading-5 text-neutral-400">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      <a
                        href="https://test-app.sodex.fi/faucet"
                        target="_blank"
                        rel="noreferrer"
                      ...
                      <a
                        href="https://docs.sodex.fi/testnet/how-to-start-trading/connect-and-fund-your-wallet"
                        target="_blank"
                        rel="noreferrer"
                  ...
                  <div className="grid gap-2 rounded-[1rem] border border-[rgba(255,255,255,0.06)] bg-[#090a0a] p-3 text-xs leading-5 text-neutral-400">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {!agentStep?.verified ? (
                        ...
                        >
                          {agentAuthorizationOpen ? "Hide authorization" : "Authorize Vega Agent"}
                        </button>
                      ) : (
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-[1rem] border border-[rgba(255,255,255,0.06)] bg-[#121416] px-3 py-3">
              <div className="grid gap-1">
                ...
                <p className="text-sm leading-5 text-neutral-400">
                  Sign in first so Vega can verify SoDEX readiness for the same wallet you&apos;ll deploy from.
                </p>
              </div>
              <button
                type="button"
