...
function extractAgentAuthorized(readiness: SoDEXReadinessPayload | null) {
...
export default function OnboardingPage() {
  ...
  const { ready, authenticated, walletAddress, getAuthHeaders, logout } = useVegaAuth();
  ...
  const refreshReadiness = useCallback(async () => {
    ...
    setError(null);
    try {
      const payload = await fetchSoDEXReadiness(walletAddress, getAuthHeaders);
      ...
      setError(
        refreshError instanceof Error
          ...
          : "Unable to verify SoDEX readiness right now.",
      );
    }
    ...
    <main className="flex min-h-screen flex-col bg-zinc-950 text-neutral-50 selection:bg-[#dce85d]/30">
      <header className="flex h-16 items-center justify-between px-6 sm:px-8">
        <Link href="/" className="inline-flex items-center gap-3 opacity-80 hover:opacity-100 transition">
          <VegaLogo className="h-5 w-5 text-neutral-50" />
          <span className="text-sm font-semibold tracking-tight">Vega</span>
        </Link>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-8 py-12">
        <div className="mx-auto w-full max-w-md">
            ...
            <div className="grid gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out-quart">
              <div className="grid gap-3">
                ...
                <p className="text-base text-neutral-400 leading-relaxed">
                  Use the same ValueChain wallet for sign-in, SoDEX funding, and agent authorization.
                </p>
              </div>
          ...
          {currentStep === 3 && (
            <div className="grid gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out-quart">
              <div className="grid gap-3">
                <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight">Reach $100 SoDEX equity</h1>
                <div className="space-y-4">
                  <p className="text-base text-neutral-400 leading-relaxed">
                    Vega uses the SoDEX protocol as its underlying trading engine. To enable automated trading, you need to fund your SoDEX account on devnet.
                  </p>
                  <ol className="list-decimal list-outside ml-4 text-sm text-neutral-400 space-y-2 marker:text-neutral-500">
                    <li>Go to the <strong className="text-neutral-200">SoDEX Faucet</strong> and mint test USDP tokens.</li>
              ...
              <div className="grid gap-5 rounded-2xl border border-white/10 bg-white/[0.02] p-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1.5 sm:flex-row sm:gap-4">
                    <ExternalAction href="https://test-app.sodex.fi/faucet" label="1. Mint USDP" />
                    <ExternalAction href="https://test-app.sodex.fi/portfolio" label="2. Deposit" />
                  </div>
                  <button
                    onClick={() => void refreshReadiness()}
                    disabled={resolvedStatus === "loading"}
          ...
          {currentStep === 4 && (
            <div className="grid gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out-quart">
              <div className="grid gap-3">
                ...
                <p className="text-base text-neutral-400 leading-relaxed">
                  Bind the delegated agent wallet so Vega can execute trades autonomously.
                </p>
              </div>
